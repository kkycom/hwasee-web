...
현재 작업을 시작할 때는 이 worktree의 목적과 요청 범위를 한 문장으로 확인한다. 이어서 `AGENTS.md`의 “장기 공유 메모”에 지정된 두 인덱스와 이번 아젠다에 관련된 메모만 읽는다. 그중 현재 작업에 영향을 주는 확정 사실·트레이드오프·회귀 금지사항을 agenda의 `기존 확정 맥락` 절에 짧게 기록한다. 다른 worktree의 작업을 추측해 수정하지 않는다.

`AGENTS.md`의 "독립 감사 필수" 대상이면, 코드를 수정하기 전에 다음을 수행한다.

1. 원래 사용자 아젠다와 `기존 확정 맥락`을 `.agent-reviews/<task-id>/agenda.md`에 기록한다. `기존 확정 맥락`에는 공유 메모에서 확인한 사실만 쓰며, 이번 작업에 대한 자신의 결론·계획은 넣지 않는다.
